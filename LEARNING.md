...
- **Daily auto-recap** — `scripts/generate-digest.mjs` runs daily 07:10 UTC (`.github/workflows/digest.yml`). Assembles a market recap from real data: BTC/ETH/SOL prices + 24h change (Coinbase daily candles), top 8 headlines (links + attribution only, never article text), next 3 events, open signals. Served at `/api/digests`, shown as "Daily Recap" card on /news. Summary is template-written by default; add `ANTHROPIC_API_KEY` repo secret (GitHub → Settings → Secrets → Actions) for AI-written prose (~$0.01/day, claude-haiku). Same-day reruns replace, keeps 30 days. Test: `node scripts/generate-digest.mjs --mock`.
- **Technical SEO** — `client/src/seo.js` hook sets per-page title/description/canonical/OG/JSON-LD (Article schema on blog posts, WebSite on home). robots.txt (blocks /admin) + sitemap.xml in `client/public/`. UPDATE `SITE_URL` in seo.js + URLs in robots.txt/sitemap.xml when custom domain is bought. After deploy: submit sitemap in Google Search Console + Bing Webmaster Tools. Remaining SEO work is content: expand 5 posts to 1,500-2,000 words, 2-3 new posts/week, backlinks (Reddit, Medium cross-posts).
